...
class ElderProfile(models.Model):
    ...
    assigned_staff = models.ManyToManyField(User, through='ElderAssignment', related_name='assigned_elders', blank=True)
...
class ElderAssignment(models.Model):
    ROLE_CHOICES = [
        ('DOCTOR', 'Doctor'),
        ('NURSE', 'Nurse'),
        ('CAREGIVER', 'Caregiver'),
    ]
    elder = models.ForeignKey(ElderProfile, on_delete=models.CASCADE, related_name='assignments')
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='elder_assignments')
