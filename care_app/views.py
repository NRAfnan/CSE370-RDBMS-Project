...
from .models import (
    ...
    VitalsLog, IncidentReport, UserProfile, ElderAssignment
)
from .forms import (
...
def get_accessible_elders(user):
    try:
        profile = user.profile
        if profile and profile.user_type in ['ADMIN', 'DOCTOR', 'NURSE', 'CAREGIVER']:
            return ElderProfile.objects.all()
    except UserProfile.DoesNotExist:
        pass
    return ElderProfile.objects.filter(Q(guardian=user) | Q(assigned_staff=user)).distinct()
...
def dashboard(request):
    ...
    if user_profile and user_profile.user_type == 'ADMIN':
        ...
        # For caregivers/doctors/nurses/guardians, show assigned or guardian elders
        elders = get_accessible_elders(request.user)
        total_elders = elders.count()
        upcoming_appointments = Appointment.objects.filter(
            elder__in=elders,
            appointment_date__gte=timezone.now(),
    ...
    # Get notifications - only for admins and guardians
    if user_profile and user_profile.user_type == 'ADMIN':
        # Admin sees all notifications
        notifications = Notification.objects.filter(is_read=False).order_by('-created_at')[:10]
    else:
        # Only guardians see notifications for their elders
        guardian_elders = ElderProfile.objects.filter(guardian=request.user)
        notifications = Notification.objects.filter(
            Q(elder__in=guardian_elders) | Q(elder__isnull=True),
            is_read=False
...
def elder_add(request):
    if request.method == 'POST':
        form = ElderForm(request.POST, user=request.user)
        if form.is_valid():
        ...
        form = ElderForm(user=request.user, initial={'guardian': request.user})
...
def elder_edit(request, elder_id):
    ...
    if request.method == 'POST':
        form = ElderForm(request.POST, instance=elder, user=request.user)
        if form.is_valid():
            form.save()
            messages.success(request, f'Elder profile for {elder.full_name} updated successfully!')
            return redirect('elder_detail', elder_id=elder.pk)
    else:
        form = ElderForm(instance=elder, user=request.user)
...
def med_schedule_add(request):
    if request.method == 'POST':
        form = MedicationScheduleForm(request.POST, user=request.user)
        if form.is_valid():
            schedule = form.save()
        ...
        elder_id = request.GET.get('elder_id')
        if elder_id:
            form = MedicationScheduleForm(initial={'elder': elder_id}, user=request.user)
        else:
            form = MedicationScheduleForm(user=request.user)
...
def appointment_add(request):
    if request.method == 'POST':
        ...
        elder_id = request.GET.get('elder_id')
        if elder_id:
            form = AppointmentForm(initial={'elder': elder_id}, user=request.user)
        else:
...
def care_task_add(request):
    if request.method == 'POST':
        form = CareTaskForm(request.POST, user=request.user)
        if form.is_valid():
            task = form.save()
        ...
        elder_id = request.GET.get('elder_id')
        if elder_id:
            form = CareTaskForm(initial={'elder': elder_id}, user=request.user)
        else:
            form = CareTaskForm(user=request.user)
...
def care_task_edit(request, task_id):
    ...
    if request.method == 'POST':
        form = CareTaskForm(request.POST, instance=task, user=request.user)
        if form.is_valid():
            form.save()
            messages.success(request, f'Task "{task.title}" updated successfully!')
            return redirect('care_task_list')
    else:
        form = CareTaskForm(instance=task, user=request.user)
...
def incident_add(request):
    if request.method == 'POST':
        ...
        elder_id = request.GET.get('elder_id')
        if elder_id:
            form = IncidentReportForm(initial={'elder': elder_id}, user=request.user)
        else:
...
def incident_edit(request, incident_id):
    ...
    if request.method == 'POST':
        form = IncidentReportForm(request.POST, instance=incident, user=request.user)
        if form.is_valid():
            form.save()
            messages.success(request, f'Incident report updated successfully!')
            return redirect('incident_list')
    else:
        form = IncidentReportForm(instance=incident, user=request.user)
...
def notification_list(request):
    try:
        ...
        # No profile means only guardian notifications
        guardian_elders = ElderProfile.objects.filter(guardian=request.user)
        notifications = Notification.objects.filter(
            Q(elder__in=guardian_elders) | Q(elder__isnull=True)
        ).order_by('-created_at')
...
def notification_delete(request, notification_id):
    ...
    try:
        ...
        if user_profile and user_profile.user_type == 'ADMIN':
            ...
            pass
        elif notification.elder and notification.elder.guardian != request.user:
            # Non-admin, non-guardian cannot delete
            messages.error(request, "You don't have permission to delete this notification.")
            return redirect('notification_list')
    except UserProfile.DoesNotExist:
        # No profile - only guardian can delete
        if notification.elder and notification.elder.guardian != request.user:
            messages.error(request, "You don't have permission to delete this notification.")
            return redirect('notification_list')
    ...
    if request.method == 'POST':
        try:
            ...
            if user_profile and user_profile.user_type == 'ADMIN':
                ...
                # Only guardians can mark their notifications as read
                guardian_elders = ElderProfile.objects.filter(guardian=request.user)
                notifications = Notification.objects.filter(
                    Q(elder__in=guardian_elders) | Q(elder__isnull=True),
                    is_read=False
...
def notification_mark_read(request, notification_id):
    ...
    try:
        ...
        if user_profile and user_profile.user_type == 'ADMIN':
            ...
            # Non-admin, non-guardian cannot mark as read
            messages.error(request, "You don't have permission to mark this notification as read.")
            return redirect('notification_list')
    except UserProfile.DoesNotExist:
        # No profile - only guardian can mark as read
        if notification.elder and notification.elder.guardian != request.user:
            messages.error(request, "You don't have permission to mark this notification as read.")
            return redirect('notification_list')
...
def user_profile(request):
    try:
        ...
        profile = UserProfile.objects.create(
            user=request.user,
            user_type='CAREGIVER'  # Default type
        )
